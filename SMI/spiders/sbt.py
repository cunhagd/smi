    ...
    def parse(self, response):
        ...
        if sel_autor:
            ...
            if autor_element:
                ...
                # Verifica se o texto contém "com informações do "
                prefixo = "com informações do "
                if autor.startswith(prefixo):
                    # Remove o prefixo e mantém apenas o nome do portal
                    autor = autor[len(prefixo):].strip()
                # Define o autor no item
                item['autor'] = autor if autor else 'Redação SBT'
            else:
                item['autor'] = 'Redação SBT'
        else:
            item['autor'] = 'Redação SBT'
